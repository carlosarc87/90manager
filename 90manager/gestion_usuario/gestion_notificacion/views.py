...
from gestion_sistema.decorators import actualizar_liga
...
def listar_notificaciones(request):
    ...
    return generar_pagina(request, "juego/notificaciones/listar.html", {"notificaciones": notificaciones})
...
def listar_notificaciones_liga(request):
    ...
    return generar_pagina(request, "juego/notificaciones/listar_liga.html", {"notificaciones": notificaciones})
...
def borrar_notificacion(request, notificacion_id):
    ...
    if usuario.notificacion_set.filter(id=notificacion_id).count() == 0:
        return devolver_mensaje(request, "Error, no existe una notificación con identificador %s" % notificacion_id)
    ...
    return devolver_mensaje(request, "La notificación ha sido eliminada", "/cuentas/notificaciones/listar_liga/")
...
def ver_notificacion(request, notificacion_id):
    ...
    # Obtenemos la notificacion
    notificacion = usuario.notificacion_set.get(id=notificacion_id)
    direccion = notificacion.get_url()
