...
class Subasta(Evento):
    ...
    def comprar(self, equipo):
        ...
        if self.tiene_comprador():
            if self.comprador != equipo:
                notificar(self.comprador.usuario, Notificacion.TIPO_NOTIFICACION.SUBASTA_SUPERADA_COMPRADA,
                          identificador=self.atributos_jugador.jugador.id, liga=self.liga)
    ...
    def finalizar(self):
        ...
        if self.tiene_comprador():
            notificar(self.comprador.usuario, Notificacion.SUBASTA_GANADA,
                      identificador=self.atributos_jugador.jugador.id, liga=self.liga)
            self.atributos_jugador.jugador.set_equipo(self.comprador)
            self.vendedor.pagar(self.puja)
        notificar(self.vendedor.usuario, Notificacion.SUBASTA_FINALIZADA,
                  identificador=self.atributos_jugador.jugador.id, liga=self.liga)
